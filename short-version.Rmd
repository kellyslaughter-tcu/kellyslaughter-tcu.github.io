...
# Short Version of Developing Visualizations from External Data

<h5>[rmd version of file](short-version.Rmd)</h5> 
...
library(dplyr)     # form inner_join() and anti_join() functions
...
Let's pull stock data on Microsoft for the last two weeks, i.e., from today (`Sys.Date()`) to two weeks ago (`Sys.Date() - 15`). The Microsoft trading symbol is **MSFT** 
...
We now have stock price data in a variable called `MSFT`. This variable is a data type called `xts`. We will change is to a data type called `tibble` using the `tidy()` command from the **broom** package and the `spread()` command from the **tidyr** package. After executing the commands, we will have a table of data called `MSFT_change` with a change of the price for each date. 

```{r Reformat-Stock-Data, message = FALSE, warning = FALSE}
...
We will get Tweets about Microsoft (up to ten thousand of the most recent over the last two weeks) using the `search_tweets()` function from the **rtweet** package. Instructions to obtain access to Twitter data by creating a _token_ can be found at [CRAN on rtweet] (https://cran.r-project.org/web/packages/rtweet/vignettes/auth.html). After executing the commands, we will have a table of data called `MSFT_tweets` with the tweet and the date/time of the tweet. 
...
Let's create a daily sentiment score for the tweets we pulled. The score will range from 5 (almost every reference to Microsoft is wonderful) to -5 (almost all the comments were negative). We will use
...
Now combine the stock and Twitter data.
